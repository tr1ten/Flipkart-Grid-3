...
df = pd.read_csv(os.path.join(base_dir,'productrec/ml/data.csv'),index_col=0)
...
def fuzzy_search(term,related_items):
    ...
    related_items.sort(key=lambda x: fuzz.partial_ratio(term,df.loc[x]['ProductTitle']),reverse=True)
    return related_items

def get_recommendation(term):
    cluster = search(term)
    return fuzzy_search(term,df[df['cluster'] == cluster].index.tolist())[:LIMIT]
...
def load_all():
    global dataset,model,nfs,ifs,id_item_map,user_feature_map
...
def update_user(user, item_id, rating):
    global nfs,ufs
    print("updating user interaction data",user['id'],item_id,rating)
    user_id = user['id']
...
def recommend(userId):
    ...
    return [id_item_map[x] for x in np.argsort(-y)[:LIMIT]]
...
if __name__ == "__main__":
    ...
    # print(n_items)
    # print(df.head())
    # print("search ",get_recommendation("shoe"))
    # new_user = {
    #     'id':3,
    #     'age': 19,
    #     'sex':'M'
    ...
    # add_user(new_user)
    load_all();
    for y in ['lehnga','white kurti ',' girls watches','headband for girls']:
        for x in get_recommendation(y):
            print(x,end=",")
    print()
        
    # print([get_item(x)['ProductTitle'] for x in (recommend(3))])
